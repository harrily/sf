...
-- 1、获取车辆信息T-2D车辆任务，
drop table if exists tmp_ordi_predict.new_require_task_info_tmp_2d_553293;
create table tmp_ordi_predict.new_require_task_info_tmp_2d_553293
...
-- 获取中转场信息
drop table if exists tmp_ordi_predict.fmsrms_dim_heavy_transit_info_df_tmp_553293;
create table tmp_ordi_predict.fmsrms_dim_heavy_transit_info_df_tmp_553293
stored as parquet as  
...
drop table if exists tmp_ordi_predict.tmp_all_arrive_before_hbase_553293;
create table tmp_ordi_predict.tmp_all_arrive_before_hbase_553293
stored as parquet as  
select
  aa.*,
...
from
  (
    select
      ...
      a.secondUnloadContnrnos as car_nos,
      a.carStatus as car_status
    from
      tmp_ordi_predict.new_require_task_info_tmp_2d_553293 a
      join tmp_ordi_predict.fmsrms_dim_heavy_transit_info_df_tmp_553293 b 
	on a.secondZoneCode = b.deptCode
    where
      a.secondZoneCode is not null
      and a.secondActualArriveTm > 0
      and a.secondUnloadContnrnos is not null
...
from
  (
    select
      ...
      a.thirdUnloadContnrnos as car_nos,
      a.carStatus as car_status
    from
      tmp_ordi_predict.new_require_task_info_tmp_2d_553293 a
      join tmp_ordi_predict.fmsrms_dim_heavy_transit_info_df_tmp_553293 b 
    ...
    where
      ...
      and a.thirdActualArriveTm > 0
      and a.thirdUnloadContnrnos is not null
  ) aa LATERAL VIEW explode(split(car_nos, ',')) adTable AS car_no
 union 
 select
  aa.*,
...
from
  (
    select
      ...
      case
        when a.destUnloadContnrnos is not null then a.destUnloadContnrnos
        when a.destArriveContnrnos is not null then a.destArriveContnrnos
        else a.carNo
      end as car_nos,
      a.carStatus as car_status
    from
    ...
    where
      a.destZoneCode is not null
      and a.destActualArriveTm > 0
...
from
  (
    select
      ...
      a.secondActualDepartTm as send_time,
      a.destZoneCode as dest_zone_code,
      a.destActualArriveTm as arrive_time,case
        when a.destUnloadContnrnos is not null then a.destUnloadContnrnos
        when a.destArriveContnrnos is not null then a.destArriveContnrnos
        else a.carNo
      end as car_nos,
