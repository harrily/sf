		...
		(
			...
			from
				...
				(select * from 
     			...
     			where carStatus in (1,2,3,4,5) and srcActualDepartTm is null) a
			join 
				-- 中转场信息
	...
	(
		...
		(
			...
			from
				...
				where carStatus in (1,2,3,4,5) and srcActualDepartTm >0 and secondActualDepartTm is null) a
			  join
				-- 中转场信息
				tmp_ordi_predict.fmsrms_dim_heavy_transit_info_df_tmp b 
			  on thirdZoneCode = b.deptCode
	...
	(
		...
		(
			...
			from
				...
				where carStatus in (1,2,3,4,5) and srcActualDepartTm >0 and secondActualDepartTm >0 and thirdActualDepartTm is null) a
			join 
			  -- 中转场信息
			tmp_ordi_predict.fmsrms_dim_heavy_transit_info_df_tmp b 
			on destZoneCode = b.deptCode
				...
				(select *,row_number() over(partition by requireId order by lastUpdateTm desc) rn
				 from 
				 	(select
							...
							case when nextzonecodedynamicprearrivetime = 'null' then null else nextzonecodedynamicprearrivetime end as nextzonecodedynamicprearrivetime,
							inc_day
						from dm_heavy_cargo.rt_vehicle_task_monitor_for_not_send_detail4 where inc_day >= '$[time(yyyyMMdd,-3d)]'
					)
				)t where t.rn = 1 
			)where carStatus in (1,2,3,4,5) and lastUpdateTm >  unix_timestamp('$[time(yyyy-MM-dd HH:mm:ss,-7d)]') * 1000   -- 取T-7D
		)
    where
      srcActualDepartTm > 0
