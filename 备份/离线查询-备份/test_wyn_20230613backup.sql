...
-- '20230101' and '20230612' 157475012
select count(1) from dm_ordi_predict.dws_static_his_cityflow_backup20230612
where inc_day between '20230101' and '20230612'
-- '20230101' and '20230612' 157475012
select count(1) from dm_ordi_predict.dws_static_his_cityflow
where inc_day between '20230101' and '20230612'

-- '20230101' and '20230612'    93745581   4606004224  4824338580
select count(1),sum(all_waybill_num) as all_waybill_num,sum(all_quantity) as all_quantity
 from dm_ordi_predict.dws_static_cityflow_base_backup20230612
where inc_day between '20230101' and '20230612'

-- '20230101' and '20230612'    93745589    4606004224  4824338580
select count(1), sum(all_waybill_num) as all_waybill_num,sum(all_quantity) as all_quantity 
from dm_ordi_predict.dws_static_cityflow_base
where inc_day between '20230101' and '20230612'

-- '20230101' and '20230612'    93745581   4606004224  4824338580
select substr(inc_day,1,4),count(1),sum(all_waybill_num) as all_waybill_num,sum(all_quantity) as all_quantity
 from dm_ordi_predict.dws_static_cityflow_base_backup20230612
group by  substr(inc_day,1,4) 
...
-- 验证是否还存在old_city_code
select * from dm_ordi_predict.dws_static_cityflow_base_backup20230612
where inc_day between '20230101' and '20230612'
and split(cityflow,'-')[0] in 
-- ,'#', 
('410','413','731','732','733','888','899','910')
or 
split(cityflow,'-')[1]  in 
('410','413','731','732','733','888','899','910') ;


select days,cityflow,income_code,weight_type_code,is_air,inc_day from dm_ordi_predict.dws_static_cityflow_base
where inc_day between '20230101' and '20230612'
and (split(cityflow,'-')[0] in 
-- ,'#', 
('410','413','731','732','733','888','899','910')
...
select days,cityflow,income_code,weight_type_code,is_air,inc_day from  dm_ordi_predict.dws_static_cityflow_base
where inc_day in ('20230112','20230117','20230118','20230208','20230217','20230405','20230415','20230530','20230606')
and cityflow in ('7311-574','7311-579','7311-027','7311-020','7311-595','7311-599','7311-579','027-7311','7311-754')




-- 丰网 262774001
-- 去丰网后 262756302  【17699丰网数据】
select count(1) from dm_ordi_predict.dws_static_his_cityflow 
where inc_day between '20200101' and '20201231'
and  !(nvl(income_code,'')='丰网' or nvl(hq_code,'')='CN39')


-- 之前备份  -- 263140706
--去丰网后 262756302   384404 丰网数据
select count(1) from  dm_ordi_predict.dws_static_his_cityflow_backup20230608
where inc_day between '20200101' and '20201231'
and !(nvl(income_code,'')='丰网' or nvl(hq_code,'')='CN39')
...
---------------------------month 月度归集 ，切换mapping----------

 select object_type ,object_type_code ,task_type ,count(1) from dm_predict.hk_cityflow_predict_month_period
 group by object_type ,object_type_code ,task_type

 select object_code,object_name  from dm_predict.hk_cityflow_predict_month_period
 where 
--  object_type =  '慢产品'
--   object_type =  '快产品'
   object_type =  '航空流向营运维度'

    select object_code,object_name,src_province,dest_province,* from dm_predict.hk_cityflow_predict_month_collecting 
