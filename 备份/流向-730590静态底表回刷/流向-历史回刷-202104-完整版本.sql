...
--* 修改人   修改时间      修改内容
/*
		-、历史数据2021-04前后的航空判断逻辑不一样。  【流向静态-历史回刷写入back表】
				-、先备份2021-04 ~ 2023-05-22月之后的数据
				-、20200101 ~ 20210331  后续在重跑备份
		-、历史2018~2019 已备份
...
(	select 
	...
	left join (select waybill_no 
              ...
              where inc_day  between '20200101' and '20201231' --  T-45D T-1D   -- 手动回刷历史数据 
              group by waybill_no
                ) b 
	on a.waybill_no = b.waybill_no
--      left join tmp_dm_predict.dws_fact_special_econ_dtl_tmp c on a.waybill_no = c.waybill_no
	where 
         a.inc_day>='20200101' and inc_day<='20201231' and   -- T-55D T-1D  -- 手动回刷历史数据 
	 substr(a.consigned_tm,0,10) between '2020-01-01' and '2020-12-31' ) a  -- 回刷历史数据 --  T-45D T-1D  -- 手动回刷历史数据 
group by  
...
select 
    ...
    */
     ,case     --202104前
		...
		when a.product_code = 'SE0089' and b.cityflow is not null  then '1'
		when a.product_code = 'SE0109' and b.cityflow is not null  then '1'
		when a.product_code = 'SE0008' and b.cityflow is not null  then '1'
		when a.product_code = 'SE0137' and b.cityflow is not null  then '1'
		when a.product_code = 'SE0121' and b.cityflow is not null  then '1'
        when a.product_code = 'SE002401' and b.cityflow is not null  then '1'
        when a.product_code = 'SE0003' and b.cityflow is not null  then '1'
		else '0' 
     end as is_air
    ...
    */
     ,case    --202104前
		...
		when a.product_code = 'SE0089' then '1'
		when a.product_code = 'SE0109' then '1'
		when a.product_code = 'SE0008' then '1'
		when a.product_code = 'SE0137' then '1'
		when a.product_code = 'SE0121' then '1'
        when a.product_code = 'SE002401' then '1'
        when a.product_code = 'SE0003' then '1'
		else '0' 
     end as is_air_waybill
from 
(   
...
SET hive.exec.dynamic.partition.mode=nonstrict;
    ...
    insert overwrite table dm_ordi_predict.dws_static_his_cityflow_backup2018_2019 partition(inc_day) 
select
    a.days
    ,a.cityflow 
    ,a.type 
...
from 
  dm_ordi_predict.dws_static_his_cityflow_backup2018_2019 
where 
    inc_day between '20200101' and '20201231'  -- 手动回刷历史数据 
group by 
    days
    ,type
    ,cityflow
...
-- insert overwrite table dm_ordi_predict.dws_static_cityflow_base partition(inc_day) 
 insert overwrite table dm_ordi_predict.dws_static_cityflow_base_backup2018_2019 partition(inc_day) 
select
    days,
    --type,
    cityflow,
