        ...
        select a.*
		 ...
		 -- 去丰网 20230612修改
		  ...
		  where (deliver_hq_code <>'CN39' or deliver_hq_code is null)
			...
			and (nvl(pickup_dept_code,'') in (select dept_code from dim.dim_department where hq_code<>'CN39' and dept_code is not null group by dept_code ) or pickup_dept_code is null )) a
          left join (
